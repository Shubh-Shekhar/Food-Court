...
export class LoginComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http
      .get<any>('http://localhost:9000/user/getAll')
      .subscribe((success) => {
  ...
  loginCheck() {
    ...
    window.localStorage.setItem(
      'loginemail',
      this.loginFormGroup.get('uemailid').value
    );
  ...
  register() {
    ...
    formData.append(
      'profile',
      new Blob([JSON.stringify(this.signupdata)], {
    ...
    this.http
      .post('http://localhost:9000/userprofile/signup', formData)
      .subscribe(
        (res) => {
          console.log(res);
