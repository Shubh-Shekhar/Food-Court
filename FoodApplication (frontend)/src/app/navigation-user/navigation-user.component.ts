...
export class NavigationUserComponent {
  ...
  constructor(
    ...
    private searchComp: SearchByComponent
  ) {}

  ngOnInit(): void {
    this.http
      .get<any>(
        'http://localhost:9000/userprofile/get/' +
    ...
    this.http
      .get<any>(
        'http://localhost:9000/addToCart/get/' +
          localStorage.getItem('loginemail'),
        { headers: reqHeader }
      )
      .subscribe((data) => {
    ...
    let reqHeader1 = new HttpHeaders().set(
      'Authorization',
      'Bearer ' + window.localStorage.getItem('token')
    );
    this.http
      .get<any>(
        'http://localhost:9000/addToFavourite/get/' +
          localStorage.getItem('loginemail'),
        { headers: reqHeader }
      )
      .subscribe((data) => {
  ...
  search() {
    ...
    if (this.searchForm.get('search').value == 'DishName')
      this.http
        .get<any>(
          'http://localhost:9000/admin/menu/searchbydishname/' +
            this.searchInput
        )
        .subscribe((success) => {
          console.log(success);
      ...
      this.http
        .get<any>('http://localhost:9000/admin/search/' + this.searchInput)
        .subscribe((success) => {
          console.log(success);
          if (success.length == 0) {
            iziToast.error({
      ...
      this.http
        .get<any>(
          'http://localhost:9000/admin/searchbycity/' + this.searchInput
        )
        .subscribe((success) => {
          console.log(success);
          if (success.length == 0) {
